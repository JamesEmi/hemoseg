...
#     y_dir = '/notebooks/triage/hemofpn/data_v2/all/masks'
    ...
    for fold, (train_ids, valid_ids) in enumerate(kfold.split(dataset)):
        ...
        checkpoint_dir = f'/notebooks/triage/hemofpn/checkpoints/{timestamp}/fold_{fold + 1}'
        # Callback to save the top 2 best checkpoints based on validation loss (or any metric you choose)
        best_checkpoint_callback = ModelCheckpoint(
            dirpath=checkpoint_dir,
            filename=f'fpn-model-dataf8-augs-lrsch-{timestamp}-fold{fold + 1}-best',
        ...
        last_checkpoint_callback = ModelCheckpoint(
            dirpath=checkpoint_dir,
            filename=f'fpn-model-dataf8-augs-lrsch-{timestamp}-fold{fold + 1}-last',
