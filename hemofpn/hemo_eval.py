...
class HemorrhageAssessment:
    ...
    def get_blood_mask(self, image):
        image_tensor = torch.tensor(np.transpose(image, (2, 0, 1))).unsqueeze(0).to(self.device)
        with torch.no_grad():
            blood_logits = self.hemobox_model(image_tensor)
        blood_mask = blood_logits.sigmoid().cpu().squeeze().numpy()
        return (blood_mask * 255).astype(np.uint8)

#     def get_person_mask_og(self, results):
#         # Find the segment ID corresponding to 'person' (assuming label ID for 'person' is 1)
#         person_segment_ids = []
#          # Iterate through the segments to find all 'person' (label_id == 0) segments
#         person_segment_id = None
#         for segment in results['segments_info']:
#             if segment['label_id'] == 0:  # '0' is label ID for 'person'
#                 person_segment_ids.append(segment['id'])

#         if not person_segment_ids:
#             self.get_logger().warn("No 'person' found in the image.")
#             return None

#         # Now compare the sizes of all the masks corresponding to these IDs
#         largest_mask = None
#         largest_area = 0
...
#         print(f"pil_image shape here is {(np.array(pil_image)).shape}")
#         padded_pil_image = self.pad_im(np.array(pil_image))  # Padding the image
#         print(f"pil_image shape here after padding is {padded_pil_image.shape}")
#         padded_pil_image_transp = np.transpose(padded_pil_image, (1, 2, 0))  # CxHxW to HxWxC
#         print(f"pil_image reshaped to {padded_pil_image_transp.shape}")
#         padded_pil_image_final = Image.fromarray(padded_pil_image_transp)  # Converting back to PIL format
#         print(f"pil_image converted back to image of shape {padded_pil_image_final.size} from array of shape {padded_pil_image_transp.shape}")
#         inputs = self.processor(images=padded_pil_image_final, return_tensors="pt").to(self.device)
#         with torch.no_grad():
#             outputs = self.mask2former_model(**inputs)
#         results = self.processor.post_process_instance_segmentation(outputs, target_sizes=[padded_pil_image_final.size[::-1]], threshold=0.9)[0]
#         segment_to_label = {segment['id']: segment['label_id'] for segment in results["segments_info"]}
        
#         person_mask = get_person_mask_og(results)
#         # if not person_segment_ids:
#         #     return None
...
#         return person_mask
    ...
    def process_image(self, image_path):
        ...
        image = self.pad_im(image)
        blood_mask = self.get_blood_mask(image)


        person_mask = self.get_person_mask(image)
        person_mask_inv = 255 - np.array(person_mask) #for viz use
        person_mask = np.array(person_mask) #for processing use
        ...
        if non_zero_pixels[0].size > 0 and non_zero_pixels[1].size > 0:
            ...
            x1, y1, x2, y2 = self.extend_box((x1, y1, x2, y2), factor=1.2, image_shape=image.shape)
            if self.zero_outside_bbox:
                blood_mask[:y1, :] = blood_mask[y2:, :] = blood_mask[:, :x1] = blood_mask[:, x2:] = 0
        else:
            print(f"No person detected in {os.path.basename(image_path)}.")
            return None

        blood_ext_ratio, overlap_ratio = self.calculate_ratios(blood_mask, person_mask)
        severe_hemorrhage = (blood_ext_ratio > 0.01) or (overlap_ratio > 0.5)

        overlay = self.overlay_mask(image, blood_mask)
        base_name = os.path.splitext(os.path.basename(image_path))[0]
        self.save_results(base_name, image, blood_mask, person_mask, overlay)
